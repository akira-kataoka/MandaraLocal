...
- **5つの地域レベル**：都道府県(47) / 市町村(1,742) / 町丁目(動的) / Shapefile / 緯度経度CSV
- **Web展開そのまま**：ビルド不要の素のHTML/CSS/JS構成。GitHub Pages で即公開。

## 実装機能（MANDARA 公式機能対比 全コンプリート）

### 主題図（14モード）
- 階級区分図（塗り分け） / 比例シンボル図 / ドット分布図 / **ラベル表示** / **円グラフ** / **棒グラフ** / **階級記号図** / **ハッチ図** / **矢印図** / **等値線図** / **カートグラム（変形地図）** / 重ね合わせ比較（2画面） / 町丁目ポイント / マルチデータセット
- 区分: 等量 / 等値 / 自然区分Jenks / **手動** + **対数スケール**
- 配色: 8パレット（YlOrRd, Blues, RdYlBu, Viridis 等）+ プレビュー + 反転

### 分析（16機能）
- 基本統計（Q1/Q3/IQR/CV/歪度/尖度/最頻値）
- 散布図 + ピアソン相関 + ログスケール
- 外れ値検出（IQR Tukey）
- 派生列計算（A op B）
- **属性検索（複数条件AND結合）**
...
- **ヒストグラム**（地図連動）
- 時系列再生 / **時空間差分・増減率派生**
- **5方向クロスハイライト**（凡例 / 散布図 / 表 / ヒストグラム / クロス集計 ↔ 地図）

### 編集（9機能）
- CSV / **Shapefile** / **緯度経度CSV** / 町丁目動的取得（Geolonia）
- **地域メッシュ生成**（1次80km / 2次10km / 3次1km）
- **ジオコーディング**（国土地理院API）
- **シーン保存**（localStorage）/ **シーンJSON共有** / **ライン編集**（Leaflet.draw）

### 出力（9形式）
- PNG / SVG / **PDF**（jsPDF）/ **アニメーションGIF**（gif.js）
- データCSV / テンプレートCSV / **KML**（Google Earth）/ **GeoJSON**（QGIS/ArcGIS）
- **URL共有**（base64 hash）

### UX
- レスポンシブ（スマホ/タブレット/PC）/ **ダークモード** / 折りたたみパネル
- 検索可能セレクタ / 配色プレビュー / 印刷向け CSS
